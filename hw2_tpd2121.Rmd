...
```{r mr trash}
mr_trashwheel = 
  read_excel("./202409_TrashWheel_Collection_ Data.xlsx", 
...
bakes = 
  read_csv(file = "./bakes.csv") %>% 
  janitor::clean_names() %>% 
  rename(first_name = baker) %>% 
  unite(id, c(series, first_name), sep = ".")

results = 
  read_csv(file = "./results.csv",
           skip = 2) %>% 
  janitor::clean_names() %>% 
  rename(first_name = baker) %>% 
  unite (id, c(series, first_name), sep = ".")
